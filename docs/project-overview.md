...
```
应用 PHP        60 文件    8,821 行      src/ routes/ templates/ bin/ bootstrap/ config/ public/
测试与工具      12 文件    1,849 行      tests/ tools/
首方前端 JS      6 文件      531 行      public/assets/js/（另含 2 个 vendored 库）
CSS              1 文件    1,007 行
────────────────────────────────────────────────────────────
数据表                    23 张
外键                      25 个
...
CHECK 约束                 5 个
路由                      45 个
提交                      19 个（原项目 17 + 本次改造 2+）
仓库跟踪文件              95 个
```

**测试与验证**
```
单元断言     78 条    跑在 SQLite 内存库，复刻生产 schema
模块校验     64 条    × 3 轮重复执行全通过
...
| **CI 绿灯** | 仓库 `Actions` 标签页 | 78 条断言在三个 PHP 版本上真实跑过 |
| **分层边界被强制** | `tools/lint.php` | "SQL 只在仓储层"是工具强制的，不是口号 |
| **一次真实的排查提交** | `git show 66acdee` | 门禁 bug 的完整修复过程与推理 |

> 最后一条尤其有效。**面试官几乎没见过候选人能展示"我的质量门禁本身有 bug，
> 我是这么发现并修复的"**——这比"我写了 8,800 行代码"更能证明工程成熟度。
